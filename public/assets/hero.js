...
(function () {
  ...
  var FRAG = [
    ...
    "  vec3 filmed = mix(f, vec3(fl), 0.70);",
    // Warm grey, not blue grey. Film on paint is dust and exhaust; tint it
    // cold and the boundary reads as a teal glow instead of as grime.
    "  filmed = mix(filmed, vec3(0.645, 0.632, 0.608), 0.42);",
    "  filmed = filmed * 0.74 + 0.115;",
    "  filmed += (streak - 0.5) * 0.075;",
    ...
    "  if (edge > 0.01) {",
    "    vec2 n = normalize(d2 + 1e-5) * 0.0011 * edge;",
    "    col.r = mix(col.r, texture2D(uTex, uv + n).r, edge * 0.34);",
    "    col.b = mix(col.b, texture2D(uTex, uv - n).b, edge * 0.34);",
    "    col += vec3(0.69, 0.55, 0.34) * edge * 0.30;",
    "  }",
  ...
  function start() {
    ...
    function frame(now) {
      ...
      if (!touched) {
        target.x = 0.5 + 0.26 * Math.sin(t * 0.34) + 0.07 * Math.sin(t * 0.71);
